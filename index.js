...
function QR8bitByte(data, binary, utf8WithoutBOM) {
    ...
    const toUTF8Array = (str) => {
        ...
        for (let i = 0; i < str.length; i++) {
            ...
            if (charcode < 0x80) {
                ...
                i++;
                const charcode2 = 0x10000 + (((charcode & 0x3ff) << 10) | (str.charCodeAt(i) & 0x3ff));
                utf8.push(
                    0xf0 | (charcode2 >> 18),
    ...
    if (binary) {
        ...
        this.parsedData = [].concat(...this.parsedData);
    } else {
    ...
    getLostPoint: function (qrCode) {
        ...
        for (var row = 0; row < moduleCount; row++) {
            for (var col = 0; col < moduleCount - 6; col++) {
                if (qrCode.isDark(row, col) && !qrCode.isDark(row, col + 1) && qrCode.isDark(row, col + 2) && qrCode.isDark(row, col + 3) && qrCode.isDark(row, col + 4) && !qrCode.isDark(row, col + 5) && qrCode
                    .isDark(row, col + 6)) {
                    lostPoint += 40;
                }
            }
        ...
        for (var col = 0; col < moduleCount; col++) {
            for (var row = 0; row < moduleCount - 6; row++) {
                if (qrCode.isDark(row, col) && !qrCode.isDark(row + 1, col) && qrCode.isDark(row + 2, col) && qrCode.isDark(row + 3, col) && qrCode.isDark(row + 4, col) && !qrCode.isDark(row + 5, col) && qrCode
                    .isDark(row + 6, col)) {
                    lostPoint += 40;
                }
            }
    ...
    function drawQrcode(oQRCode) {
        for (var row = 0; row < nCount; row++) {
            for (var col = 0; col < nCount; col++) {
                ...
                var eye = oQRCode.getEyeBlock(row, col); // { isDark: true/false, type: PO_TL, PI_TL, PO_TR, PI_TR, PO_BL, PI_BL };
        ...
        if (_htOption.subTitle) {
            ...
            _oContext.fillText(_htOption.subTitle, t._canvas.width / 2, _htOption.quietZone + _htOption.subTitleTop);
        }
        ...
        if (_htOption.logo) {
            ...
            function generateLogoImg(img) {
                ...
                var imgContainerY = (_htOption.realWidth - imgContainerH) / 2;
...
Drawing.prototype.makeImage = function () {
    ...
    switch (makeOptions.makeType) {
        case 'FILE':
            if (this._htOption._drawer === 'svg') {
                ...
                const stream = this._htOption.format === 'PNG'
                    ? this._canvas.createPNGStream({ compressionLevel: this._htOption.compressionLevel })
                    : this._canvas.createJPEGStream({ quality: this._htOption.quality });
        ...
        case 'STREAM':
            const stream = this._htOption.format === 'PNG'
                ? this._canvas.createPNGStream()
                : this._canvas.createJPEGStream();
            this.resolve(stream);
            break;
    }
};
...
function QRCode(vOption) {
    ...
    if (this._htOption.format == 'PNG' && (this._htOption.compressionLevel < 0 || this._htOption.compressionLevel > 9)) {
        console.warn(this._htOption.compressionLevel + " is invalidate, PNG compressionLevel must between 0 and 9, now reset to 6. ")
        this._htOption.compressionLevel = 1;
    } else if (this._htOption.quality < 0 || this._htOption.quality > 1) {
        console.warn(this._htOption.quality + " is invalidate, JPG quality must between 0 and 1, now reset to 0.75. ")
